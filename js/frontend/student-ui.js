...
async function renderInternshipsBoard(typeFilter = 'All') {
  ...
  container.innerHTML = filtered.map((opp, idx) => `
    <div class="p-5 rounded-2xl bg-white dark:bg-gray-900/70 border border-[#E2E8F0] dark:border-purple-500/30 hover:border-purple-400 dark:hover:border-purple-500/60 transition shadow-sm flex flex-col justify-between space-y-4">
      <div>
        <div class="flex justify-between items-start gap-2">
          ...
          <span class="text-xs font-mono font-bold text-emerald-600 dark:text-cyan-300">${opp.match}% Skill Fit</span>
        </div>

        <h3 class="font-bold text-base text-[#0F172A] dark:text-white mt-2 mb-0.5">${opp.title}</h3>
        <p class="text-xs text-purple-600 dark:text-purple-300 font-medium">${opp.company}</p>
  ...
  container.innerHTML = jobs.map((opp, idx) => `
    <div class="p-6 rounded-3xl bg-white dark:bg-gray-900/80 border border-[#E2E8F0] dark:border-blue-500/40 hover:border-blue-400 dark:hover:border-blue-500 transition shadow-sm flex flex-col justify-between space-y-4">
      <div>
        ...
        <p class="text-xs text-blue-600 dark:text-blue-300 font-semibold">${opp.company} • ${opp.location}</p>
      ...
      <div class="pt-4 border-t border-[#E2E8F0] dark:border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <span class="text-[10px] text-slate-400 dark:text-gray-500 block">Deadline: ${opp.deadline}</span>
        </div>
  ...
  if (!quizState.started) {
    container.innerHTML = `
      <div class="text-center py-12 space-y-4 max-w-md mx-auto">
        ...
        <p class="text-xs sm:text-sm text-slate-600 dark:text-gray-400">Validate pharmacognosy and bio-data science competencies. Earn up to +250 XP to boost recruiter rankings.</p>
  ...
  if (quizState.finished) {
    ...
    container.innerHTML = `
      <div class="text-center py-12 space-y-4 max-w-md mx-auto">
        ...
        <h3 class="text-2xl font-black text-[#0F172A] dark:text-white">Quiz Completed!</h3>
        <p class="text-sm text-slate-600 dark:text-gray-300">Score: <strong class="text-emerald-600 dark:text-emerald-400 font-mono text-lg">${quizState.score} / ${QUIZ_DATA.length}</strong></p>
        <div class="p-3.5 rounded-2xl bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-500/30 text-purple-700 dark:text-purple-300 text-xs font-mono">
          +${xpWon} XP Awarded to Student Profile!
        </div>
        <button onclick="startQuiz()" class="px-6 py-2.5 rounded-xl bg-[#0f172a] hover:bg-[#1e293b] dark:bg-purple-600 dark:hover:bg-purple-500 text-white text-xs font-bold transition shadow-sm">
          Retake Quiz ↺
        </button>
      </div>
    `;
  ...
  container.innerHTML = `
    <div class="space-y-5 max-w-xl mx-auto py-4">
      <div class="flex justify-between items-center text-xs text-slate-500 dark:text-gray-400">
        ...
        <span class="font-mono text-purple-600 dark:text-purple-400 font-bold">+50 XP</span>
      </div>
      <div class="w-full bg-slate-100 dark:bg-gray-900 rounded-full h-1.5 overflow-hidden">
        <div class="bg-purple-600 h-full transition-all" style="width: ${((quizState.currentIndex + 1) / QUIZ_DATA.length) * 100}%"></div>
      </div>
      <h3 class="text-base sm:text-lg font-bold text-[#0F172A] dark:text-white">${q.question}</h3>
        ...
        ${q.options.map((opt, i) => `
          <button onclick="selectQuizAnswer(${i})" class="w-full p-3.5 rounded-2xl text-left text-xs sm:text-sm transition flex items-center justify-between ${
            quizState.selectedAnswer === i 
              ? 'bg-purple-600 border border-purple-400 text-white font-bold shadow-sm' 
            ...
            <span>${opt}</span>
            <span class="w-4 h-4 rounded-full border border-slate-300 dark:border-gray-700 flex items-center justify-center text-[10px]">
              ${quizState.selectedAnswer === i ? '✓' : ''}
            </span>
          </button>
        `).join('')}
      ...
      <div class="flex justify-end pt-3">
        <button onclick="nextQuizQuestion()" class="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition shadow-sm">
